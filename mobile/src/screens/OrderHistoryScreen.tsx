...
type Props = MainDrawerScreenProps<'OrderHistory'>;
...
const CANCELLABLE_STATUSES: OrderResponse['status'][] = ['PENDING_PAYMENT', 'PAID', 'PREPARING'];
...
export default function OrderHistoryScreen({ route }: Props) {
  const [confirmationCode, setConfirmationCode] = useState(route.params?.confirmationCode ?? '');
  ...
  useEffect(() => {
    if (route.params?.confirmationCode) {
      setConfirmationCode(route.params.confirmationCode);
    }
  }, [route.params?.confirmationCode]);
  ...
  async function handleCheckStatus() {
    ...
    if (!trimmedEmail) {
      nextErrors.guestEmail = 'Enter the email used to order.';
    ...
    setLoading(true);
    try {
      const result = await lookupOrder({ confirmationCode: trimmedCode, guestEmail: trimmedEmail });
      setOrder(result);
    } catch (error) {
    ...
    >
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.subheading}>Look up your order with your confirmation code</Text>

          {route.params?.status === 'success' && !order && (
            <View style={styles.returnBanner}>
              ...
              <Text style={styles.returnBannerText}>
                Payment complete. Enter your email to check the order status.
              </Text>
            </View>
          )}
          {route.params?.status === 'cancelled' && !order && (
            <View style={[styles.returnBanner, styles.returnBannerWarning]}>
              <Ionicons name="information-circle-outline" size={16} color={colors.danger} />
              <Text style={[styles.returnBannerText, styles.returnBannerWarningText]}>
                Checkout was cancelled — your order is still awaiting payment. Enter your email to
                check or retry.
              </Text>
            </View>
          )}
          ...
          <View style={styles.fieldGroup}>
            <Text style={styles.label}>
              <Ionicons name="receipt-outline" size={13} color={colors.textMuted} /> Confirmation
              Code
            </Text>
          ...
          <View style={styles.fieldGroup}>
            ...
            <View style={[styles.inputBox, errors.guestEmail && styles.inputBoxError]}>
              ...
              <TextInput
                ...
                placeholder="Enter the email used to order"
                placeholderTextColor={colors.textMuted}
                style={styles.textInput}
          ...
          {notFound && (
            <View style={styles.notice}>
              ...
              <Text style={styles.noticeText}>
                No order matches that code and email. Double-check both and try again.
              </Text>
            </View>
          )}
          ...
          {order && (
            <View style={styles.resultCard}>
              <View style={styles.resultHeader}>
                <Text style={styles.resultCode}>{order.confirmationCode}</Text>
              ...
              {CANCELLABLE_STATUSES.includes(order.status) && !showCancelForm && (
                <TouchableOpacity
